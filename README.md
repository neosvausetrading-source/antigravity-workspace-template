...
**Searchable use cases:** AI workspace template, MCP server for Claude Code, Codex CLI project context, Cursor repository Q&A, Windsurf agent memory, Gemini CLI coding assistant, codebase knowledge graph, GitNexus-powered code search, and multi-agent software development.
...
## Who Uses Antigravity?

Antigravity is for developers who want an **AI IDE starter kit** instead of scattered prompt files. It gives Claude Code, Codex CLI, Cursor, Windsurf, Gemini CLI, Cline, Aider, and VS Code Copilot the same reusable project knowledge, so coding agents can answer architecture, dependency, implementation, and git-history questions from real repository evidence.

Use it when you need:
- **Repository Q&A** — ask how a module works and get answers grounded in file paths, line numbers, and generated `agents/*.md` knowledge.
- **MCP context engine** — expose `ask_project` and `refresh_project` to Claude Code or any MCP-capable AI coding workflow.
- **Multi-agent documentation** — run `ag-refresh` to generate module knowledge, routing maps, conventions, structure, and git insights.
- **Cross-IDE AI memory** — keep `.antigravity/` as the shared knowledge layer for Claude Code, Codex, Cursor, Windsurf, Gemini CLI, Cline, Aider, and Copilot.
- **Graph-aware code search** — combine semantic module docs with optional GitNexus call graph, dependency, and impact analysis.
...
| `ag-ask "question"` | Router → ModuleAgent/GitAgent routed Q&A | Calling main agent when embedded |
| `ag-mcp --workspace <dir>` | **Start MCP server** — exposes `ask_project` + `refresh_project` to Claude Code | Host main agent |
